...
class Apimodel extends CI_Model
{
    ...
    function getCoveragedetails($id)
    {
        $data = $this->db->get_where('master_coverage', array('master_policy_id' => $id))->result();
        return $data;
    }
	...
	function getProductsList($post)
	{
		...
		$default_sort_order = 'desc';
		$condition = "i.isactive=1 AND ij.isactive=1";
		...
		if ($query->num_rows() >= 1) {
			...
			foreach ($query->result() as $key => $value) {
				$value->URL='partner='.encrypt_decrypt_password($value->creditor_id,'E').'&product='.encrypt_decrypt_password($value->plan_id,'E');
				$result[]=$value;
			}
			return array("query_result" => $result, "totalRecords" => $totcount);
		} else {
			return array("totalRecords" => 0);
		}
	}
	...
	{
        /*$this->db->select('creditor_id,creditor_name');
        $this->db->from('master_ceditors');
		$data = $this->db->where(array('isactive' => 1))->result();*/
         $data=$this->db->query("select creditor_id,creaditor_name from master_ceditors where isactive=1 order by creaditor_name asc")->result();
	...
	{
		$this->db->select('mp.*,master_plan.creditor_id,master_plan.gender,master_plan.duplicate_coi_allow,master_plan.coi_start_series,master_plan.series_digit_count,master_plan.coi_download,master_plan.is_single_coi,master_plan.pan_mandatory,master_plan.nominee_mandatory,master_plan.plan_name,master_plan.coi_type,master_plan.policy_type_id,mpst.policy_sub_type_name,mc.creaditor_name as creditor_name,payment_first,payment_page');
	...
	{
		$this->db->select('mp.*,master_plan.is_valid_age,master_plan.self_mandatory,master_plan.creditor_id,master_plan.gender,master_plan.plan_name,master_plan.product_type_id,mpst.policy_sub_type_name,mpst.code as policy_sub_type_code,mpsi.suminsured_type_id as sitype_id,mppb.si_premium_basis_id as basis_id,mc.creaditor_name as creditor_name,mc.creditor_logo');
		$this->db->where('mp.plan_id', $id);
		$this->db->where('mp.isactive', 1);
	...
	function getProductDetailsAll_diff($id, $sort_by = '', $order = '', $policy_id_arr = [])
	{
		...
		$this->db->select('mp.*,master_plan.creditor_id,master_plan.self_mandatory,master_plan.gender,master_plan.plan_name,master_plan.product_type_id,mpst.policy_sub_type_name,mpst.code as policy_sub_type_code,mpsi.suminsured_type_id as sitype_id,mppb.si_premium_basis_id as basis_id,mc.creaditor_name as creditor_name,mc.creditor_logo,master_plan.payment_first,payment_page,master_plan.gender,(select group_concat(member_type_id) from master_policy_family_construct fc where fc.master_policy_id=mp.policy_id and isactive=1) as family_construct');
	...
	function getSiPremiumBasisNew($policy_type_id)
	{
        $this->db->like('policy_type', $policy_type_id);
		$data = $this->db->get_where('master_si_premium_basis', array('isactive' => 1))->result();
		return $data;
	}
	...
	function getMembers($gender='B')
	{
		switch ($gender) {
			case 'F':
				$gender_con = "gender is NULL OR gender='Female'";
				break;
			case 'M':
				$gender_con = "gender is NULL OR gender='Male'";
				break;
			
			default:
				$gender_con =  "gender is NULL OR gender='Female' OR gender='Male'";
				break;
		}
		$sql = $this->db->query("select * from family_construct where  isactive = 1  and (".$gender_con.")" );
		if ($sql->num_rows() > 0) {
			return $sql->result();
		} else {
			return false;
	...
	***/
	...
	$premium = $rate->premium_rate;
	 $premium_with_tax = $rate->premium_with_tax;
		...
		$rate->premium_with_tax = number_format($premium_with_tax, 2, '.', '');
...
//echo 123;die;
        ...
        if($data['planwise_premium_api'] == 'Yes') {
             // echo 67;
            foreach ($rate as $key => $value) {
                // $rate = $rate;
                $rates = $this->hydrateEmptyRates($value);

                $response['rate'][$key]['rate'] = (!empty($rates->premium_with_tax) && $rates->premium_with_tax != '0.00') ? $rates->premium_with_tax : $rates->premium_rate;;
                $response['rate'][$key]['rate_without_tax'] = $rates->premium_rate;
               //$response['rate'][$key]['deductable'] = $value->deductable;
                $response['status'] = true;
                $response['rate'][$key]['group_code'] = $rate->group_code;
                $response['rate'][$key]['group_code_spouse'] = $rate->group_code_spouse;
               // $response['rate'][$key]['cover'] = $value->sum_insured;
            ...
            $response['rate'] = $rate->premium_with_tax + $rate->premium_rate;
	...
	function getPolicyPerDayTenurePremium($data)
	{
		...
		$response['rate'] = (!empty($rate->premium_with_tax) && $rate->premium_with_tax!='0.00') ? $rate->premium_with_tax : $rate->premium_rate;;
		$response['rate_without_tax'] = $rate->premium_rate;
		$response['status'] = true;
		$response['group_code'] = $rate->group_code;
		$response['group_code_spouse'] = $rate->group_code_spouse;
...
//print_r($this->db->last_query());
	...
	function getPolicyFamilyDeductable($data)
	{
		...
		try {
			...
			else if($data['deductable'] !='' && $data['deductable'] !=0){

				$rate = $this->apimodel->getdata1("master_policy_premium", "*", " master_policy_id = " . $data['policy_id'] .  " AND min_age <= " . $data['age'] . " AND max_age >= " . $data['age'] . " AND  sum_insured = " . $data['sum_insured'] . " AND adult_count = " . $data['adult_count'] . " AND child_count = " . $data['child_count'] . " AND deductable=" . $data['deductable'] . " and isactive=1");

			}
			else if(!empty($data['deductable_si']) && $data['deductable'] != 1)
            {
                $rate = $this->apimodel->getdata1("master_policy_premium", "*", " master_policy_id = " . $data['policy_id'] .  " AND min_age <= " . $data['age'] . " AND max_age >= " . $data['age'] . " AND adult_count = " . $data['adult_count'] . " AND child_count = " . $data['child_count'] .  " AND min_si <= " . $data['deductable_si'] . " AND max_si >= " . $data['deductable_si']."  and isactive=1");

            }
			else{

                $rate = $this->apimodel->getdata1("master_policy_premium", "*", " master_policy_id = " . $data['policy_id'] .  " AND min_age <= " . $data['age'] . " AND max_age >= " . $data['age'] ." AND  sum_insured = " . $data['sum_insured'] . " AND adult_count = " . $data['adult_count'] . " AND child_count = " . $data['child_count'] . "  and isactive=1");
		...
		if($data['planwise_premium_api'] == 'Yes') {
		 //   echo 67;
            foreach ($rate as $key => $value) {
                ...
                if ($value->child_count != 0) {
                    if ($value->child_count == 1) {
                        $response['rate'][$key]['construct'] = $value->adult_count . "+" . $value->child_count . "Kid";
                    } else {
                        $response['rate'][$key]['construct'] = $value->adult_count . "+" . $value->child_count . "Kids";
                    }
                } else {
                    $response['rate'][$key]['construct'] = $value->adult_count;
                }
                $response['rate'][$key]['cover'] = $value->sum_insured;
            ...
            $response['group_code'] = $rate->group_code;
           // $response['group_code'] = $rate->group_code;
            $response['member_code'] = $rate->member_code;
...
//print_r($this->db->last_query());
	...
	//	print_r($rate);
		...
		$rate = $this->hydrateEmptyRates($rate);
//print_r($rate);
        if($rate->is_taxable == 1)
        {
            $rate_amt = $rate->premium_with_tax + $rate->premium_rate;
        }else {
            $rate_amt = $rate->premium_rate;
          //  print_r($rate_amt);die;
        }

		$response['rate'] = round($rate_amt);
		$response['rate_without_tax'] = round($rate_amt);
...
//print_r($data['sum_insured']);
		$amount = ($data['sum_insured'] / 1000) * $rate[0]->policy_rate;
//print_r($rate[0]->policy_rate);

        if($rate[0]->is_taxable == 1){
            $final_rate = $amount  *  (100 + (int) getConfigValue('tax_percent')) / 100;
        }else{
            $final_rate = $amount;
        }
		$response['rate'] = number_format($final_rate, 2, '.', '');
