   ...
   //     $data = (curlFunction(SERVICE_URL . '/customer_api/getCOiinfo', $req_data));
    ...
    //    $data['coi_html']=$coi_html;
      //  $coi_html=$data['coi_html'];
        ...
        $coi_html = str_replace("{{sum_insured}}", $premium_details['sum_insured'], $coi_html);
        ...
        $coi_html = str_replace("{{policy_start_date}}", date('jS F Y',strtotime($policy_start_date)), $coi_html);

        if($data_marine['status']== 200){
            extract($data_marine['data'][0]);
         //   print_r($Bill_number);die;
            $start_date = date('Y-m-d',strtotime($date_of_shipment));
            $end_date = date('Y-m-d', strtotime($start_date . ' + 365 days'));
            $coi_html = str_replace("{{bill_number}}", $Bill_number, $coi_html);
            $Bill_date_replace = !empty($Bill_date)?date('d/m/Y',strtotime($Bill_date)):"";
            $coi_html = str_replace("{{bill_date}}", $Bill_date_replace, $coi_html);
            ...
            $coi_html = str_replace("{{dateofissuance}}", date('d/m/Y',strtotime($created_date)), $coi_html);
            $coi_html = str_replace("{{invoice_number}}", $Invoice_number , $coi_html);
            $coi_html = str_replace("{{startdate}}", "00:01 hrs ".$start_date, $coi_html);
            $coi_html = str_replace("{{enddate}}", "23:59 on ".$end_date, $coi_html);
...
//echo 1;
    ...
    function is_mandatory($data){
        $_POST=$data;
        $response = 'true';
        if(empty($_POST['Plan_id'])){
            $res = array('success' => false, 'msg' => "Plan Id  is required");
            $response = json_encode($res);
           // exit;
        }

        if(empty($_POST['Cust_First_Name'])){
            $res = array('status' => false, 'msg' => "Customer First Name is required");
            $response = json_encode($res);

        }
        if(empty($_POST['Cust_Last_Name'])){
            $res = array('status' => false, 'msg' => "Customer Last Name is required");
            $response = json_encode($res);

        }
        if(empty($_POST['Cust_Mobile_No'])){
            $res = array('status' => false, 'msg' => "Customer Mobile Number is required");
            $response = json_encode($res);

        }

        if(empty($_POST['Sum_Insured'])){
            $res = array('status' => false, 'msg' => "Sum Insured is required");
            $response = json_encode($res);

        }
        if(empty($_POST['Insurer_Name'])){
            $res = array('status' => false, 'msg' => "Insurer Name is required");
            $response = json_encode($res);

        }
        if(empty($_POST['Invoice_Amount'])){
            $res = array('status' => false, 'msg' => "Invoice Amount is required");
            $response = json_encode($res);

        }

        if(empty($_POST['Product_Name'])){
            $response = array('status' => false, 'msg' => "Product Name is required");
            $response = json_encode($res);

        }
        if(empty($_POST['Master_Policy_No'])){
            $res = array('status' => false, 'msg' => "Master policy number is required");
            $response = json_encode($res);

        }
        if(empty($_POST['Invoice_No'])){
            $res = array('status' => false, 'msg' => "Invoice number is required");
            $response = json_encode($res);

        }
        if(empty($_POST['Invoice_Date'])){
            $res = array('status' => false, 'msg' => "Invoice Date is required");
            $response = json_encode($res);

        }
        if(empty($_POST['Lead_Id'])){
            $res = array('status' => false, 'msg' => "Lead id is required");
            $response = json_encode($res);

        }

        if(empty($_POST['Product_Name'])){
            $res = array('status' => false, 'msg' => "Product Name is required");
            $response = json_encode($res);

        }
        if(empty($_POST['Master_Policy_No'])){
            $res = array('status' => false, 'msg' => "Master Policy No is required");
            $response = json_encode($res);

        }
        if(empty($_POST['Weight_of_gold'])){
            $res = array('status' => false, 'msg' => "Weight Of Gold is required");
            $response = json_encode($res);

        }
        if(empty($_POST['Ornaments_Details'])){
            $res = array('status' => false, 'msg' => "Ornaments Details is required");
            $response = json_encode($res);
    ...
    function allriskApi()
    {
      /*  ini_set('display_errors', 1);
        ...
        if(isset($_POST['json_cust_data'])){
            ...
            $is_redirection_api=true;
        }else{
            $api_data = json_decode(file_get_contents('php://input') , true);
            $api_data_json = file_get_contents('php://input');
            // print_r($api_data_json);die;
            $lan_number =   $api_data['QuoteRequest']['LoanDetails']['LoanAccountNo'];
            //     insert_redirection_api_log($lan_number, "redirection_json", $api_data_json);

        }
        $token = $api_data['token'];
        $checkToken=$this->db->query("select emp_id,time  from token_table t where token= '".$token."' and type=3")->row();
          //print_r($checkToken);die;
        if(empty($checkToken) || is_null($checkToken)){
            $response = array('status' => false, 'msg' => "Unauthorized Access!");
            echo json_encode($response);
            exit;
        }else{
            $time=$checkToken->time;
            $minutes = (time() - strtotime($time)) / 60;
            if($minutes > 40){
                $response = array('status' => false, 'msg' => "Token Expired!");
                echo json_encode($response);
                exit;
            }
        ...
        $_POST = $api_data;
       $is_mandatory = $this->is_mandatory($_POST);
       if($is_mandatory != 'true')
       {
        ...
        $check_invoice = $this->db->query("select * from risk_customer_info where invoice_no = '$invoice_no'")->row_array();
        if(!empty($check_invoice))
        {
            $response = array('status' => false, 'msg' => "This Invoice Number is already exist.");

            echo json_encode($response);
            exit;
        }
        $cust_data['plan_id'] = (!empty($_POST['Plan_id'])) ? $_POST['Plan_id'] : '';

        $cust_data['customer_id'] = (!empty($_POST['Customer_ID'])) ? $_POST['Customer_ID'] : '';
        $cust_data['dob'] = (!empty($_POST['Cust_DOB'])) ? date("Y-m-d", strtotime($_POST['dob'])) : '';
        $cust_data['insurer_name'] = (!empty($_POST['Insurer_Name'])) ? $_POST['Insurer_Name'] : '';
        $cust_data['salutation'] = (!empty($_POST['Cust_Salutation'])) ? $_POST['Cust_Salutation'] : '';
        $cust_data['first_name'] = (!empty($_POST['Cust_First_Name'])) ? $_POST['Cust_First_Name'] : '';
        $cust_data['middle_name'] = (!empty($_POST['Cust_Middle_Name'])) ? $_POST['Cust_Middle_Name'] : '';
        $cust_data['last_name'] = (!empty($_POST['Cust_Last_Name'])) ? $_POST['Cust_Last_Name'] : '';

        $cust_data['mobile_no'] = (!empty($_POST['Cust_Mobile_No'])) ? $_POST['Cust_Mobile_No'] : '';
        $cust_data['email_id'] = (!empty($_POST['Cust_email_ID'])) ? $_POST['Cust_email_ID'] : '';
        $cust_data['gender'] = (!empty($_POST['Cust_Gender'])) ? $_POST['Cust_Gender'] : '';
        $cust_data['pincode'] = (!empty($_POST['Cust_Pincode'])) ? $_POST['Cust_Pincode'] : '';

        //$cust_data['email_id'] = (!empty($_POST['Cust_email_ID'])) ? $_POST['Cust_email_ID'] : '';
        $cust_data['createdon'] = date("Y-m-d H:i:s");
        $cust_data['createdby'] = (!empty($_POST['sm_id'])) ? $_POST['sm_id'] : '';
        $cust_data['address1'] = (!empty($_POST['Cust_Add1'])) ? $_POST['Cust_Add1'] : '';
        $cust_data['address2'] = (!empty($_POST['Cust_Add2'])) ? $_POST['Cust_Add2'] : '';
        $cust_data['sum_insured'] = (!empty($_POST['Sum_Insured'])) ? $_POST['Sum_Insured'] : '';
        $cust_data['invoice_amount'] = (!empty($_POST['Invoice_Amount'])) ? $_POST['Invoice_Amount'] : '';
        $cust_data['policy_start_date'] = (!empty($_POST['Policy_Start_Date'])) ? $_POST['Policy_Start_Date'] : '';
        $cust_data['policy_end_date'] = (!empty($_POST['Policy_End_Date'])) ? $_POST['Policy_End_Date'] : '';
        $cust_data['product_name'] = (!empty($_POST['Product_Name'])) ? $_POST['Product_Name'] : '';
        $cust_data['master_policy_number'] = (!empty($_POST['Master_Policy_No'])) ? $_POST['Master_Policy_No'] : '';
        $cust_data['invoice_no'] = (!empty($_POST['Invoice_No'])) ? $_POST['Invoice_No'] : '';
        $cust_data['invoice_date'] = (!empty($_POST['Invoice_Date'])) ? $_POST['Invoice_Date'] : '';
        $cust_data['policy_tenure'] = (!empty($_POST['Policy_Tenure'])) ? $_POST['Policy_Tenure'] : '';
        $cust_data['nominee_first_name'] = (!empty($_POST['Nominee_first_name'])) ? $_POST['Nominee_first_name'] : '';
        $cust_data['nominee_last_name'] = (!empty($_POST['Nominee_last_name'])) ? $_POST['Nominee_last_name'] : '';
        $cust_data['nominee_relation'] = (!empty($_POST['Nominee_Relation'])) ? $_POST['Nominee_Relation'] : '';
        $cust_data['nominee_dob'] = (!empty($_POST['Nominee_DOB'])) ? $_POST['Nominee_DOB'] : '';
        $cust_data['nominee_mobile'] = (!empty($_POST['Nominee_Mobile'])) ? $_POST['Nominee_Mobile'] : '';
        $cust_data['customer_lead_id'] = (!empty($_POST['Lead_id'])) ? $_POST['Lead_id'] : '';
        $cust_data['trace_id'] = (!empty($_POST['Trace_id'])) ? $_POST['Trace_id'] : '';
        $cust_data['ornaments_details'] = (!empty($_POST['Ornaments_Details'])) ? $_POST['Ornaments_Details'] : '';
        $cust_data['weight_of_gold'] = (!empty($_POST['Weight_of_gold'])) ? $_POST['Weight_of_gold'] : '';

        $plan_id = $cust_data['plan_id'];
        $get_policy_details = $this->db->query("select * from master_policy where plan_id = '$plan_id'")->result_array();
        foreach($get_policy_details as $policy_details){

            $policy_id = $policy_details['policy_id'];
            $policy_subtype_id = $policy_details['policy_sub_type_id'];
            $creadior_id = $policy_details['creditor_id'];
            $coverage_type = $policy_details['coverage_type'];
            if($coverage_type == 'single') {
                $coverage_data = $this->db->query("select * from master_coverage where master_policy_id = '$policy_id'")->row_array();
                if($coverage_data['premium_type'] == 'percentage'){
                    $premium = ($cust_data['sum_insured'] * $coverage_data['premium_rate'])/100;
                }else{
                    $premium = $cust_data['sum_insured'] * $coverage_data['premium_rate'];
                }
            }

        }
        $cust_data['total_premium'] = $premium;
        $d_l_r=array(
            'mobile' => $cust_data['mobile_no'],
            'email' =>  $cust_data['email_id'],
            'creditor_id' => $creadior_id,
            'gender' => $cust_data['gender'],
            'plan_id' => $plan_id,
            'name' => $cust_data['first_name']." " .$cust_data['last_name'],
            'Address1' => $cust_data['address1'],
            'Address2' => $cust_data['address2'],
        ...
        $req_dataN['trace_id'] = encrypt_decrypt_password($checkDetails['data']['trace_id'], 'D');
        $req_dataN['plan_id'] = $plan_id;
        $req_dataN['nominee_name'] =   $cust_data['nominee_first_name'] ." ". $cust_data['nominee_last_name'];
        $req_dataN['nominee_relation'] = 1;
        ...
        $cust_data['lead_id'] =   $req_dataN['lead_id'];

        //$customer_id = $this->apimodel->insertData('risk_customer_info', $cust_data, 1);
        $customer_id =  $this->db->insert("risk_customer_info", $cust_data);
        ...
        $req_data['trace_id'] = $checkDetails['data']['trace_id'];
        $req_data['plan_id'] = $plan_id;
        $req_data['cover'] =$cust_data['sum_insured'];
        $req_data['tenure'] = 1;
        ...
        if ($proposal_data == true) {
            ...
            $req_data1['TransactionNumber'] = $cust_data['invoice_no'];
            $req_data1['TransactionRcvdDate'] = $cust_data['invoice_date'];
            $q_creditor_id= $this->db->query("select creditor_id,ceditor_email,creaditor_name from master_ceditors where creditor_id = '$creadior_id' ")->row_array();
            $creditor_id=$q_creditor_id['creditor_id'];
            $ceditor_email=$q_creditor_id['ceditor_email'];
            $creaditor_name=$q_creditor_id['creaditor_name'];
            //cover limit code
            ...
            if ($PaymentMode == 4) {
                ...
                if($response_cd['status'] == 201){
                    ...
                    if($response_cd['msg']=="NegativeAllow"){
                        ...
                        $response = array('status' => true, 'msg' => "Success", 'data' => array());
...
if($coi_type == 2)
{
    ...
    $enddate = date('Y-m-d H:i:s', strtotime("-1 day", strtotime($new_date)));
    $end_date = date('Y-m-d H:i:s', strtotime("+1 year", strtotime($enddate)));
    $fqrequest = [
        "masterPolicyClient" => "AllRisk",
        "insuredAddress"=> $cust_data['address1'],

            "insuredName" =>  $cust_data['first_name'] ." " .$cust_data['last_name'],
            "insuredEmail" => $cust_data['email_id'],
            "insuredMobile" => $cust_data['mobile_no'],
            "invoiceRequestNumber" => $cust_data['invoice_no'],
            "invoiceDate" => date('m/d/Y',strtotime($cust_data['invoice_date'])),
            "sumInsured" =>$cust_data['sum_insured'],
            "extraAttrib01" => $cust_data['ornaments_details'],
            "insuredProductCode" =>"Jewellery",
            "insuredProductName" => $cust_data['product_name'],
            "insuredProductBrandName" => "",
            "eWDealerCode" => "",
            "customerId" => $cust_data['customer_id'],
            "itemDetail" =>$cust_data['weight_of_gold'],
            "numberofItems" => "1",
            "riskEndDate" => date('m/d/Y', strtotime("-1 day", strtotime($new_date))),
            "riskStartDate" => date('m/d/Y'),
            "insurancePlan" => "",



    ];
    //print_r(json_encode($fqrequest));
    insert_application_log(encrypt_decrypt_password($checkDetails['data']['lead_id'], 'D'), 'full_quote_request', json_encode($fqrequest), "", 123);

    $insert_id = $this
        ->db
        ->insert_id();
//print_r($get_token['access_token']);die;
    $curl = curl_init();

    curl_setopt_array($curl, array(
        CURLOPT_URL => 'https://staging.iffcotokio.co.in/partner-services/ins/allrisk',
        CURLOPT_RETURNTRANSFER => true,
        CURLOPT_ENCODING => '',
        CURLOPT_MAXREDIRS => 10,
        CURLOPT_TIMEOUT => 0,
        ...
        CURLOPT_HTTPHEADER => array(
            ...
            'Authorization: Basic SVRHSVBITjAwMTpwYXJ0bmVyQDIwMjA='
    ...
    $request_arr = ["response_data" => $response];
    $this
        ->db
...
if(!empty($res['certificate_spcNumber'])) {
    $data_apr = array(
        'lead_id' => $req_dataN['lead_id'],
        'policy_sub_type_id'=>$policy_subtype_id,
        'customer_id' => $req_dataN['customer_id'],
        'master_policy_id' => $policy_id,
        'certificate_number' => $res['certificate_spcNumber'],
        'gross_premium' => $premium,
        'start_date' => $start_date,
        'end_date' => $end_date,
        'ReceiptNumber' => $res['receipt_spcNumber'],
        'ProposalNumber' => $res['declaration_spcId']

    );

       $this->db->where('lead_id',$req_dataN['lead_id']);
    $this->db->update('api_proposal_response', $data_apr);
}else{
    $response = array('status' => false, 'msg' => $res['status']);
    echo json_encode($response);
    exit;
            ...
            $get_proposal_number=$this->db->query("select certificate_number from api_proposal_response where lead_id=".$req_dataN['lead_id'])->row()->certificate_number;
            insert_application_log($req_dataN['lead_id'], 'Policy Created successfully', json_encode("Success"),json_encode("Lead ID:- " . $req_dataN['lead_id'] . "--" . "Trace ID:- " . $req_dataN['trace_id']. "--" . "Certificate Number:- " . $get_proposal_number), 123);
            echo "Lead ID:- " . $req_dataN['lead_id'] . "--" . "Trace ID:- " . $req_dataN['trace_id'] . "--" . "Certificate Number:- " . $get_proposal_number;

            if ($update_payment_status->StatusCode == 200) {
               // echo 123;die;
                $response['status'] = true;
               // $response['redirection_url'] =  FRONT_URL . '/quotes/success_view/'.$req_data1['lead_id'].'?lead_id='.$req_data1['lead_id'];
//echo json_encode($response);
               // redirect('/quotes/success_view/'.$req_data1['lead_id'].'?lead_id='.$req_data1['lead_id']);
    ...
    public function rehit_icici_policy_issuance($type)
    {
        if($type == 1) {
            $query = $this->db->query("select pp.*,ppd.*,mc.creditor_id from proposal_payment_details  as ppd join proposal_policy as pp join master_ceditors as mc where ppd.lead_id = pp.lead_id and ppd.creditor_id = mc.creditor_id  and ppd.status = 'Pending' AND pp.cron_count < 2 AND  mc.api_lable_id = 'icici_lombard' AND date(pp.created_at) = date(now())")->result_array();

            foreach ($query as $details) {
                $lead_id = $details['lead_id'];
                $cust_data = $this->db->query("select * from lead_details where lead_id = '$lead_id'")->row_array();
                $name = $cust_data['first_name'] . " " . $cust_data['last_name'];

                $response = $this->createProposal_icici($details['lead_id'], $name, $cust_data);
                insert_application_log($lead_id, 'proposal_coi_issuance_cron_icici', json_encode($response), "", 123);

            }
        }else{
            $query = $this->db->query("select apr.* from proposal_payment_details  as ppd join proposal_policy as pp join master_ceditors as mc join api_proposal_response as apr where ppd.lead_id = pp.lead_id and ppd.creditor_id = mc.creditor_id  and apr.lead_id = pp.lead_id and ppd.status = 'Payment-Done' AND pp.cron_count < 2 AND  mc.api_lable_id = 'icici_lombard' and  apr.coi_url = '' AND date(pp.created_at) = date(now())")->result_array();
            foreach ($query as $details) {
                ...
                $name = $cust_data['first_name'] . " " . $cust_data['last_name'];
                $res = [];
                $res->confirmedCertificateNumber = $query['certificate_number'];
                $res->proposalNumber = $query['proposalNumber'];
                $v4uuid = $query['uuid'];
                $response = $this->get_coi_icici($res, $lead_id, $v4uuid);
                insert_application_log($lead_id, 'coi_issuance_cron_icici', json_encode($response), "", 123);
    ...
    {//echo 123;die;
     ...
     error_reporting(E_ALL);*/
        ...
        $_POST = $api_data; // print_r($api_data);die;
        $ClientCreation = $_POST['ClientCreation'];
        $RiskDetail = $_POST['RisksDetails'];
        $RiskDetails = $RiskDetail[0];
        ...
        if ($Api_type == 'issuance' || $Api_type == 'modification') {
            ...
            $response_cover = CheckCoverBalance($creditor_id,$plan_id,$policy_id,0);

            $cover_data = $response_cover['data'];
            $cover_limit =$cover_data['cover_limit'];
            $cover_balance=$cover_data['balance'];
            $cover_utilised=$cover_data['amount_utilzed'];
            $cover_collection_amt=$cover_data['collection_amount'];


            //cover check data
            if($response_cover['status'] != 0) {
                if ($response_cover['status'] == 201) {
                    $mail = sendMailCoverbalance($ceditor_email, $creaditor_name, $cover_limit, $cover_balance, $cover_collection_amt, $response_cover['msg']);
                    echo $response_cover['msg'];
                    die;
                } else {
                    if ($response_cover['msg'] == "LessCover") {
                        $mail = sendMailCoverbalance($ceditor_email, $creaditor_name, $cover_limit, $cover_balance, $cover_collection_amt, $response_cover['msg']);
            ...
            $data_arr_cd=$response_cd['data'];
            $cd_threshold=$data_arr_cd['threshold_amount'];
            $cd_utilised=$data_arr_cd['amount_utilzed'];
            ...
            $business_type = $ClientCreation['Business_type'];
            $limit_details  = $this->db->query("select excess,business_type,per_sending_limit,per_location_limit,b2c_type,per_sending_limit_b2c,per_location_limit_b2c from master_policy where policy_id = '$policy_id'")->row_array();
         //print_r($this->db->last_query());die;
            $Excess = $limit_details['excess'];

            //   echo $req_dataN['lead_id'];die;
            $req_dataN['plan_id'] = $plan_id;
            $req_dataN['nominee_name'] = $name;
            $req_dataN['nominee_relation'] = 1;
            $checkDetailsMM = curlFunction(SERVICE_URL . '/customer_api/updateNomineeDetails', $req_dataN);
            insert_application_log($req_dataN['lead_id'], 'Proposer Details', json_encode($req_dataN),json_encode($checkDetailsMM), 123);
            //echo encrypt_decrypt_password($checkDetails['data']['lead_id'], 'D');die;
            $get_plan_det = $this->db->query("select api_label_id from master_plan where plan_id = '$plan_id'")->row_array();
            //print_r($RiskDetails);die;
            if($get_plan_det['api_label_id'] == 'icici_lombard'){
                $pancard = $ClientCreation['PANCardNo'];
                $mobileisd = $ClientCreation['MobileISD'];
                $gstdetails = $ClientCreation['GSTDetails'];
                $adharno = $ClientCreation['AadharNumber'];
                $collection_form = $ClientCreation['IsCollectionofform60'];
                $adhar_enroll = $ClientCreation['AadharEnrollmentNo'];
                $eia_number = $ClientCreation['eIA_Number'];
                $customer_id = $ClientCreation['CustomerID'];
                $ckycid = $ClientCreation['CKYCId'];
                $ekycid = $ClientCreation['EKYCid'];
                $pepflag = $ClientCreation['PEPFlag'];
                $ilkycref_no = $ClientCreation['ILKYCReferenceNumber'];
                $skipdedupelogic = $ClientCreation['SkipDedupeLogic'];
                $data_of_incoporation = $ClientCreation['DateOfIncorporation'];
                $source_of_funds = $ClientCreation['SourceOfFunds'];
                $other_of_funds = $ClientCreation['OtherFunds'];
                $cin = $ClientCreation['CIN'];
                $Excess = $ClientCreation['Excess'];
                $BalanceCargoSIShowInCert = $RiskDetails['BalanceCargoSIShowInCert'];
                $Basis_of_valuation = $RiskDetails['BasisofValuation'];
                $BlawrrlrDate= $RiskDetails['BlawrrlrDate'];
                $BlawbrrlRNo = $RiskDetails['BlawbrrlRNo'];
                $CargoSI = $RiskDetails['CargoSI'];
                $CargoSICurr = $RiskDetails['CargoSICurr'];
                $CertificateGenerateDate = $RiskDetails['CertificateGenerateDate'];
                $Currency = $RiskDetails['Currency'];
                $CurrencyRate = $RiskDetails['CurrencyRate'];
                $CustomValueSI = $RiskDetails['CustomValueSI'];
                $CustomValueSICurr = $RiskDetails['CustomValueSICurr'];
                $ExpImpoNameAndAddress = $RiskDetails['ExpImpoNameAndAddress'];
                $FinancialInterest = $RiskDetails['FinancialInterest'];
                $GoodDescription = $RiskDetails['GoodDescription'];
                $IntermediaryStorageSI = $RiskDetails['IntermediaryStorageSI'];
                $IntermediaryStorageSICurr = $RiskDetails['IntermediaryStorageSICurr'];
                $LCNo = $RiskDetails['LCNo'];
                $LoadingPremiumrate = $RiskDetails['LoadingPremiumrate'];
                $LoadingWarSRCCrate = $RiskDetails['LoadingWarSRCCrate'];
                $LocationFrom = $RiskDetails['LocationFrom'];
                $LocationTo = $RiskDetails['LocationTo'];
                $ModeOfTransitAir = $RiskDetails['ModeOfTransitAir'];
                $ModeOfTransitCourier = $RiskDetails['ModeOfTransitCourier'];
                $ModeOfTransitPostal = $RiskDetails['ModeOfTransitPostal'];
                $ModeOfTransitRail = $RiskDetails['ModeOfTransitRail'];
                $ModeOfTransitRoad = $RiskDetails['ModeOfTransitRoad'];
                $ModeOfTransitSea = $RiskDetails['ModeOfTransitSea'];
                $PackagingDetails = $RiskDetails['PackagingDetails'];
                $PackagingDetailsOther = $RiskDetails['PackagingDetailsOther'];
                $PlaceOfDischarge = $RiskDetails['PlaceOfDischarge'];
                $PlaceOfLoading = $RiskDetails['PlaceOfLoading'];
                $PolicySubType = $RiskDetails['PolicySubType'];
                $PortofDischarge = $RiskDetails['PortofDischarge'];
                $PortofLoading = $RiskDetails['PortofLoading'];
                $PremiumShownInCert = $RiskDetails['PremiumShownInCert'];
                $RiskCommensementDate = $RiskDetails['RiskCommensementDate'];
                $ServiceTaxShownInCert = $RiskDetails['ServiceTaxShownInCert'];
                $SettlementAgentTown = $RiskDetails['SettlementAgentTown'];
                $SettlingAgentCountry = $RiskDetails['SettlingAgentCountry'];
                $SettlingAgentTown = $RiskDetails['SettlingAgentTown'];
                $ShipmentRemarks = $RiskDetails['ShipmentRemarks'];
                $ShipmentTravelsvia = $RiskDetails['ShipmentTravelsvia'];
                $TypeMarksAndContainerNo = $RiskDetails['TypeMarksAndContainerNo'];
                $TotalPackages = $RiskDetails['TotalPackages'];
                $VoyageNo = $RiskDetails['VoyageNo'];
                $WeightofGoods = $RiskDetails['WeightofGoods'];
                $limit_per_sending = $ClientCreation['LimitPerSending'];
                $limit_per_location = $ClientCreation['LimitsPerLocation'];

                if(!empty($limit_details))
                {
                    if($business_type == 'B2B') {
                        if ($limit_per_sending > $limit_details['per_sending_limit']) {
                            echo "Per sending limit Not greater than " . $limit_details['per_sending_limit'];
                            exit;
                        }
                        if ($limit_per_location > $limit_details['per_location_limit']) {
                            echo "Per location limit Not greater than " . $limit_details['per_location_limit'];
                            exit;
                        }
                    }
                    if($business_type == 'B2C')
                    {
                        if ($limit_per_sending > $limit_details['per_sending_limit_b2c']) {
                            echo "Per sending limit Not greater than " . $limit_details['per_sending_limit_b2c'];
                            exit;
                        }
                        if ($limit_per_location > $limit_details['per_location_limit_b2c']) {
                            echo "Per location limit Not greater than " . $limit_details['per_location_limit_b2c'];
                            exit;
                        }
                    }
                }
            ...
            $data_marine = array(
                ...
                'date_of_shipment' => !empty($date_of_shipment)?date('Y-m-d',strtotime($date_of_shipment)):"",
                'Bill_number' => $Bill_number,
                'Bill_date' =>!empty($Bill_date)?date('Y-m-d H:i:s',strtotime($Bill_date)):"",
                'credit_number' => $credit_number,
                'credit_description' => $credit_description,
                'place_of_issuence' => $place_of_issuence,
                'Invoice_number' => $Invoice_number,
                'Invoice_date' =>!empty($Invoice_date)?date('Y-m-d H:i:s',strtotime($Invoice_date)):"",
                ...
                'IsCollectionofform60'=>$collection_form,
                'AadharEnrollmentNo'=>$adhar_enroll,
                ...
                'EKYCid'=>$ekycid,
                'PEPFlag'=>$pepflag,
                'ILKYCReferenceNumber'=>$ilkycref_no,
                'SkipDedupeLogic'=>$skipdedupelogic,
                'DateOfIncorporation'=>$data_of_incoporation,
                'SourceOfFunds'=>$source_of_funds,
                'OtherFunds'=>$other_of_funds,
                'CIN'=>$cin,
                'BalanceCargoSIShowInCert'=>$BalanceCargoSIShowInCert,
                'BlawrrlrDate'=>$BlawrrlrDate,
                'BlawbrrlRNo'=>$BlawbrrlRNo,
                'CargoSI'=>$CargoSI,
                'CargoSICurr'=>$CargoSICurr,
                'CertificateGenerateDate'=>$CertificateGenerateDate,
                'CustomValueSICurr'=>$CustomValueSICurr,
                'Currency'=>$Currency,
                'CurrencyRate'=>$CurrencyRate,
                'CustomValueSI'=>$CustomValueSI,
                'ExpImpoNameAndAddress'=>$ExpImpoNameAndAddress,
                'CustomValueSICurr'=>$CustomValueSICurr,
                'FinancialInterest'=>$FinancialInterest,
                'GoodDescription'=>$GoodDescription,
                'IntermediaryStorageSI'=>$IntermediaryStorageSI,
                'IntermediaryStorageSICurr'=>$IntermediaryStorageSICurr,
                'LCNo'=>$LCNo,
                'PolicySubType'=>  $PolicySubType,
                'PortofDischarge'=>$PortofDischarge,
                'PortofLoading' =>$PortofLoading,
                'LoadingPremiumrate'=>$LoadingPremiumrate,
                'LoadingWarSRCCrate'=>$LoadingWarSRCCrate,
                'LocationFrom'=>$LocationFrom,
                'LocationTo'=>$LocationTo,
                'ModeOfTransitAir'=>$ModeOfTransitAir,
                'ModeOfTransitCourier'=>$ModeOfTransitCourier,
                'ModeOfTransitPostal'=>$ModeOfTransitPostal,
            'ModeOfTransitRail'=>$ModeOfTransitRail,
            'ModeOfTransitRoad'=>$ModeOfTransitRoad,
            'ModeOfTransitSea'=>$ModeOfTransitSea,
            'PackagingDetails'=>$PackagingDetails,
            'PackagingDetailsOther'=>$PackagingDetailsOther,
            'PlaceOfDischarge'=>$PlaceOfDischarge,
            'PlaceOfLoading'=>$PlaceOfLoading,
            'PremiumShownInCert'=>$PremiumShownInCert,
            'RiskCommensementDate'=>$RiskCommensementDate,
            'ServiceTaxShownInCert'=>$ServiceTaxShownInCert,
            'SettlementAgentTown'=>$SettlementAgentTown,
            'SettlingAgentCountry'=>$SettlingAgentCountry,
            'SettlingAgentTown'=>$SettlingAgentTown,
            'ShipmentRemarks'=>$ShipmentRemarks,
            'TotalPackages'=>$TotalPackages,
            'TypeMarksAndContainerNo'=>$TypeMarksAndContainerNo,
            'VoyageNo'=>$VoyageNo,
                'WeightofGoods'=>$WeightofGoods,
                'limit_per_sending'=>$limit_per_sending,
                'limit_per_location'=>$limit_per_location,
                'business_type'=>$business_type

            );
            //print_r($data_marine);die;
...
//print_r($Api_type);die;
                if ($Api_type == 'modification') {
                    $last_cover = $this->db->query("select cover from policy_member_plan_details where lead_id = '$lead_id'")->row()->cover;

                    $cover_balance_remain = $cover_balance + $last_cover;
                    $policy_cover_data['cd_balance_remain'] = $cover_balance_remain;
                    $policy_cover_data['cd_utilized'] = $cover_utilised - $last_cover;
                    $policy_insert_cover_data = $this->updateRecord('master_policy', $policy_cover_data, "policy_id='" . $policy_id . "'");
                    ...
                    $update_cd_data = curlFunction(SERVICE_URL . '/customer_api/cover_credit_debit_entry', $cd_data);
                    //cover debit_credit transaction
                }
                else{
                   // echo 123;die;

                    $response_cover = CheckCoverBalance($creditor_id,$plan_id,$policy_id,$cargo_value);
                   // print_r($response_cover);die;
                    if($response_cover['status'] != 0) {
                        $cover_data = $response_cover['data'];
                        $cover_limit = $cover_data['cover_limit'];
                        $balance = $cover_data['balance'];
                        $cover_collection_amt = $cover_data['collection_amount'];
                        if ($response_cover['status'] == 201) {
                            $mail = sendMailCoverbalance($ceditor_email, $creaditor_name, $cover_limit, $cover_balance, $cover_collection_amt, $response_cover['msg']);
                            echo $response_cover['msg'];
                        ...
                        //print_r($cover_data);die;
                        $update_cover_data = curlFunction(SERVICE_URL . '/customer_api/cover_credit_debit_entry', $cover_data);
                    }
                }
                if ($PaymentMode == 4) {
                    if ($Api_type == 'modification') {
                ...
                if($get_plan_det['api_label_id'] == 'icici_lombard') {
                   // echo 456;die;
                    ...
                    $response = $this->createProposal_icici($req_data['lead_id'],$name,$d_l_r);
print_r($response);die;
            ...
            if($result){
                $cover_details = $this->db->query("select initial_cover  from master_policy  where policy_id = '$policy_id' and isactive=1")->row_array();

                if(!empty($cover_details['initial_cover']) &&  $cover_details['initial_cover'] != 0) {
                    $last_cover = $this->db->query("select cover from policy_member_plan_details where lead_id = '$lead_id' order by id desc limit 1 ")->row()->cover;
                    ...
                    $cover_balance_remain = $cover_balance + $last_cover;
                    $policy_cover_data['cover_balance_remain'] = $cover_balance_remain;
                    $policy_cover_data['cover_utilized'] = $cover_utilised - $last_cover;
                    $policy_cancel_data = $this->updateRecord('master_policy', $policy_cover_data, "policy_id='" . $policy_id . "'");
                    $cover_data = array(
                        'type' => 1,
                        'amount' => $last_cover,
                        'lead_id' => $lead_id,
    ...
    function createProposal_icici($lead_id,$name,$cust_data)
    {
      //  echo 123;die;
        $v4uuid = UUID::v4();
//print_r($v4uuid);die;
        $lead_id = encrypt_decrypt_password($lead_id,'D');
        $scope = 'esbmarinecertificate';
        $data =   $this->generate_token_icici($scope);
        $get_token = json_decode($data,true);
       // print_r($get_token['access_token']);
       // print_r($get_token['access_token']);

        $get_marine_details = $this->db->query("select * from marine_customer_info where lead_id = '$lead_id'")->row_array();
      //print_r($get_marine_details);die;
        $risk_det[] = [
          "DeclareBalanceCargoSIShowInCert" =>$get_marine_details['BalanceCargoSIShowInCert'],
          "DeclareBasisofValuation"=>$get_marine_details['Basis_of_valuation'],
          "DeclareBlawrrlrDate"=>$get_marine_details['BlawrrlrDate'],
          "DeclareBlawbrrlRNo"=>$get_marine_details['BlawbrrlRNo'],
          "DeclareCargoSI"=>$get_marine_details['CargoSI'],
          "DeclareCargoSICurr"=>$get_marine_details['CargoSICurr'],
          "DeclareCertificateGenerateDate"=>$get_marine_details['CertificateGenerateDate'],
          "DeclareConsigneeNameAddress"=>$get_marine_details['Consignee_add'],
          "DeclareCurrency"=>$get_marine_details['Currency'],
          "DeclareCurrencyRate"=>$get_marine_details['CurrencyRate'],
          "DeclareCustomerName"=>$name,
          "DeclareCustomValueSI"=>$get_marine_details['CustomValueSI'],
          "DeclareCustomValueSICurr"=>$get_marine_details['CustomValueSICurr'],
          "DeclareExcess"=>$get_marine_details['Excess'],
          "DeclareExpImpoNameAndAddress"=>$get_marine_details['ExpImpoNameAndAddress'],
          "DeclareFinancialInterest"=>$get_marine_details['FinancialInterest'],
          "DeclareGoodDescription"=>$get_marine_details['GoodDescription'],
          "DeclareIntermediaryStorageSI"=>$get_marine_details['IntermediaryStorageSI'],
          "DeclareIntermediaryStorageSICurr"=>$get_marine_details['IntermediaryStorageSICurr'],
          "DeclareInvoiceDate"=>$get_marine_details['Invoice_date'],
          "DeclareInvoiceNo"=>$get_marine_details['Invoice_number'],
          "DeclareLCNo"=>$get_marine_details['LCNo'],
          "DeclareLoadingPremiumrate"=>$get_marine_details['LoadingPremiumrate'],
          "DeclareLoadingWarSRCCrate"=>$get_marine_details['LoadingWarSRCCrate'],
          "DeclareLocationFrom"=>$get_marine_details['LocationFrom'],
          "DeclareLocationTo"=>$get_marine_details['LocationTo'],
          "DeclareModeOfTransitAir"=>$get_marine_details['ModeOfTransitAir'],
          "DeclareModeOfTransitCourier"=>$get_marine_details['ModeOfTransitCourier'],
          "DeclareModeOfTransitPostal"=>$get_marine_details['ModeOfTransitPostal'],
          "DeclareModeOfTransitRail"=>$get_marine_details['ModeOfTransitRail'],
          "DeclareModeOfTransitRoad"=>$get_marine_details['ModeOfTransitRoad'],
          "DeclareModeOfTransitSea"=>$get_marine_details['ModeOfTransitSea'],
          "DeclarePackagingDetails"=>$get_marine_details['PackagingDetails'],
          "DeclarePackagingDetailsOther"=>$get_marine_details['PackagingDetailsOther'],
          "DeclarePlaceOfDischarge"=>$get_marine_details['PlaceOfDischarge'],
          "DeclarePlaceOfLoading"=>$get_marine_details['PlaceOfLoading'],
          "DeclarePolicySubType"=>$get_marine_details['PolicySubType'],
          "DeclarePortofDischarge"=>$get_marine_details['PortofDischarge'],
          "DeclarePortofLoading"=>$get_marine_details['PortofLoading'],
          "DeclarePremiumShownInCert"=>$get_marine_details['PremiumShownInCert'],
          "DeclareRiskCommensementDate"=>$get_marine_details['RiskCommensementDate'],
          "DeclareServiceTaxShownInCert"=>$get_marine_details['ServiceTaxShownInCert'],
          "DeclareSettlementAgentTown"=>$get_marine_details['SettlementAgentTown'],
          "DeclareSettlingAgentCountry"=>$get_marine_details['SettlingAgentCountry'],
          "DeclareSettlingAgentTown"=>$get_marine_details['SettlingAgentTown'],
          "DeclareShipmentRemarks"=>$get_marine_details['ShipmentRemarks'],
          "DeclareShipmentTravelsvia"=>$get_marine_details['ShipmentTravelsvia'],
          "DeclareTotalPackages"=>$get_marine_details['TotalPackages'],
          "DeclareTypeMarksAndContainerNo"=>$get_marine_details['TypeMarksAndContainerNo'],
          "DeclareVesselName"=>$get_marine_details['vessel_name'],
          "DeclareVoyageNo"=>$get_marine_details['VoyageNo'],
          "DeclareWeightofGoods"=>$get_marine_details['WeightofGoods'],
      ];
$riskdetails[] = ["Risks"=>$risk_det];

        $fqrequest = [
                        "PolicyNumber" => "2002/52691707/00/000",
                        "ProductCode"=>2425,
                        "CustomerDetail" => [
                                                "CustomerType" => "",
                                                "CustomerName" => $name,
                                                "DateOfBirth" => "",
                                                "PinCode" => $cust_data['pincode'],
                                                "PANCardNo" => $get_marine_details['PANCardNo'],
                                                "Email" =>$cust_data['email'],
                                                "MobileNumber" => $cust_data['mobile'],
                                                "AddressLine1" => $cust_data['Address1'],
                                                "AddressLine2" => $cust_data['Address2'],
                                                "CountryCode" => null,
                                                "StateCode" => "",
                                                "CityCode" => "",
                                                "Gender" =>$cust_data['gender'],
                        ...
                        "RisksDetails" => $riskdetails,
                         "CorrelationId"=> $v4uuid,
...
//print_r($get_token['access_token']);die;
        ...
        curl_setopt_array($curl, array(
            CURLOPT_URL => 'https://ilesbapigee.insurancearticlez.com/misc-products/ilservices/products/v1/marinecertificate/createproposal',
            CURLOPT_RETURNTRANSFER => true,
            CURLOPT_ENCODING => '',
            CURLOPT_MAXREDIRS => 10,
            CURLOPT_TIMEOUT => 0,
      ...
      //print_r($response);
        ...
        if($res->statusMessage == 'FAILED')
        {
            ...
            if(empty($res->confirmedCertificateNumber))
            {
                echo "Failed to load policy pdf, as certificate number not received in response";
                exit;
               // echo "Failed to load Pdf due to Certificate Number Not Received";
            }
            asyncCurlFunction(FRONT_URL . '/GadgetInsurance/get_coi_icici_async',['CertificateNumber'=>$res->confirmedCertificateNumber,'proposalNumber'=>$res->proposalNumber,'lead_id'=>$lead_id,'v4uuid'=>$v4uuid,'invoice_number'=>$get_marine_details['Invoice_number']]);
            //$get_coi = $this->get_coi_icici($res,$lead_id,$v4uuid);
            //print_r($get_coi);
            $response = array('status' => 'success','message'=>'You will receive coi shortly on mail');
...
function get_coi_icici($res,$lead_id,$v4uuid)
{
    //$v4uuid = UUID::v4();
   ...
   // $res = json_decode($response);
  // print_r($res);die;
    $scope = 'esbmarinepolicycertificate';
    $data =   $this->generate_token_icici($scope);
    $get_token = json_decode($data);
    //print_r($get_token);
    $fqrequest = ["CorrelationId"=> $v4uuid,"CertificateNumber"=>$res->confirmedCertificateNumber,"customerId"=>"100289712366"];
 //print_r($fqrequest['CertificateNumber']);
    insert_application_log($lead_id, 'coi_download_request', json_encode($fqrequest), "", 123);
    $insert_id = $this
        ->db
        ->insert_id();

    $api_proposal = ["certificate_number" => $res->confirmedCertificateNumber,"ProposalNumber"=>$res->proposalNumber,"uuid"=>$v4uuid];
    $this
        ->db
        ->where("lead_id", $lead_id);
    $this
        ->db
        ->update("api_proposal_response", $api_proposal);
 //   $res = json_decode($responses);

    $curl = curl_init();

    curl_setopt_array($curl, array(
        CURLOPT_URL => 'https://ilesbapigee.insurancearticlez.com/misc-products/ilservices/products/v1/marine/policycertificate',
        CURLOPT_RETURNTRANSFER => true,
        CURLOPT_ENCODING => '',
        CURLOPT_MAXREDIRS => 10,
        CURLOPT_TIMEOUT => 0,
    ...
    curl_close($curl);
   // die;
    $request_arr = ["response_data" => $responses];
    $this
        ->db
        ->where("log_id", $insert_id);
    $this
    ...
    if($res->statusMessage == 'FAILED')
    {
      ...
      //  echo 123;die;
        $file = './assets/marine_coi/marineIC_' . str_replace('/', '_', $fqrequest['CertificateNumber']) . '.pdf';
      // print_r($fqrequest['CertificateNumber']);
    //  print_r($file);

        $coi_url = ["COI_url" => $file];
        $this
            ->db
            ->where("lead_id", $lead_id);
        $this
            ->db
            ->update("api_proposal_response", $coi_url);
        $data = file_put_contents($file, $responses);
        $coi_link  = FRONT_URL . '/assets/marine_coi/marineIC_'. str_replace('/', '_', $fqrequest['CertificateNumber']) . '.pdf';
        $response = array('status' => 'success', 'url' => $coi_link);
...
function get_coi_icici_async()
{
    //$v4uuid = UUID::v4();
   
    $lead_id = $_REQUEST['lead_id'];
    $v4uuid = $_REQUEST['v4uuid'];
    $CertificateNumber = $_REQUEST['CertificateNumber'];
    $proposalNumber = $_REQUEST['proposalNumber'];
    ...
    $get_token = json_decode($data);
    //print_r($get_token);
    $fqrequest = ["CorrelationId"=> $v4uuid,"CertificateNumber"=>$CertificateNumber,"customerId"=>"100289712366"];
 //print_r($fqrequest['CertificateNumber']);
    insert_application_log($lead_id, 'coi_download_request', json_encode($fqrequest), "", 123);
    $insert_id = $this
        ->db
    ...
    $api_proposal = ["certificate_number" => $CertificateNumber,"ProposalNumber"=>$proposalNumber,"uuid"=>$v4uuid];
    $this
        ->db
        ->where("lead_id", $lead_id);
    $this
 ...
 //   $res = json_decode($responses);
   ...
   // die;
    ...
    if ($resultStatus == 200) {
        if($res->statusMessage == 'FAILED')
        {
          ...
          // print_r($fqrequest['CertificateNumber']);
            ...
            $data = file_put_contents($file, $responses);
            $coi_link  = FRONT_URL . '/assets/marine_coi/marineIC_'. str_replace('/', '_', $fqrequest['CertificateNumber']) . '.pdf';
            $request = array('invoice_number' => $invoice_number, 'COI_URL' => $coi_link);
            
            
            insert_application_log($lead_id, 'coi_webhook_request', json_encode($request), "", 123);
            $insert_id = $this
                    ->db
                    ->insert_id();
            $curl = curl_init();
            ...
            curl_setopt_array($curl, array(
                CURLOPT_URL => 'https://testapi.boxnmove.com/insurance/gadget/webhook',
                CURLOPT_RETURNTRANSFER => true,
                CURLOPT_ENCODING => '',
                CURLOPT_MAXREDIRS => 10,
                CURLOPT_TIMEOUT => 0,
    ...
    function generate_token_icici($scope)
    {
        $curl = curl_init();

        curl_setopt_array($curl, array(
            CURLOPT_URL => 'https://ilesbapigee.insurancearticlez.com/generate-jwt-token',
            CURLOPT_RETURNTRANSFER => true,
            CURLOPT_ENCODING => '',
            CURLOPT_MAXREDIRS => 10,
            CURLOPT_TIMEOUT => 0,
            ...
            CURLOPT_HTTP_VERSION => CURL_HTTP_VERSION_1_1,
            CURLOPT_CUSTOMREQUEST => 'POST',
            CURLOPT_POSTFIELDS => 'grant_type=password&username=AllianceIns&password=pgGxG6vcAfMFCsJ&client_id=AllianceIns&scope='.$scope.'&client_secret=l77wN5sWRwBIEWEGKS7pszKdXSQAzJXGQqeYG7GeLu95MSGJvIGg8mDBFYw2GLzM',
            CURLOPT_HTTPHEADER => array(
    ...
    function send_customer_data($lead_id,$status){
        $query=$this->db->query("select * from marine_procedure_data where date(created_at) = date(now())");
        if($this->db->affected_rows() > 0){
            $result=$query->result();
            $this->load->library('excel');
 ...
 from marine_procedure_data mpd where (created_at) >= '".$yesterday."' AND created_at <= now()")->result();
        ...
        foreach ($query as $key => $row) {
            ...
            if($row->subject_matter_insured =='Car' && $row->type_of_shipment=="Intra"){
                $arrayHouseoldIntra = array(
                    ...
                    $row->to_city, $row->cargo_value, $row->date_of_shipment,
                    $row->vessel_name, //"Transporter details"
                    $row->subject_matter_insured,$row->plan_id,$row->status,$row->certificate_number,$row->certificate_number
                );
                $char = 'A';
                foreach ($arrayHouseoldIntra as $k => $r) {
    ...
    function SendFTPmail($response_array){
            ...
            try {
                ...
                $mail->SMTPSecure = "tls";
                $mail->Username   = 'Cert.nobroker@allianceinsurance.in';                     //SMTP username
                $mail->Password   = 'wpkseiftsewyqscv';                               //SMTP password
                // $mail->SMTPSecure = PHPMailer::ENCRYPTION_SMTPS;            //Enable implicit TLS encryption
                $mail->Port       = 587;                                    //TCP port to connect to; use 587 if you have set `SMTPSecure = PHPMailer::ENCRYPTION_STARTTLS`
...
//echo $mail_array[0];die;
        ...
        try {
            ...
            $mail->addBCC("iafzalkhan007@gmail.com");
...
";
    ...
    function send_policy_request_to_ic_re(){
        ini_set('display_errors', 1);
        ini_set('display_startup_errors', 1);
        error_reporting(E_ALL);
        $post_data='{
    "token": "eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9.eyJ1c2VybmFtZSI6IjUxIiwiaWF0IjoxNjg2ODA1Mjk2LCJleHAiOjE2ODY4MjMyOTZ9.VdOShHuxnCVbGZGgElFnVZ_TQwiDcLoGPQEhvpboLCg",
    "ClientCreation": {
        "partner": " CHINMAY FINLEASE LIMITED",
        "plan": "GPA GCI EMI LOE",
        "salutation": "Mr",
        "first_name": "Amihjt",
        "middle_name": "",
        "last_name": "Matani",
    ...
    "PolicyCreationRequest": {
        ...
        "TransactionRcvdDate": "10-12-2023",
        "CollectionAmount": "468.52",
    ...
    "data": null,
    "mem_resp": [
        "{\"status_code\":\"200\",\"Metadata\":{\"Message\":\"Record Updated.\",\"member_id\":3297,\"data_added\":\"1\"}}"
    ],
    "Nominee": {
        "status_code": "200",
        "Metadata": {
    ...
    "Payment": "{\"status_code\":\"200\",\"Metadata\":{\"Message\":\"Record Updated Successfully\"}}",
    "Premium": {
        "status": true,
        "messages": {
            "Applicant": {
        ...
        "data": {
            "Applicant": {
                ...
                "total_premium": "468.12"
            },
            "net_premium": "468.12"
        }
    },
    "LeadId": "RmlqUUlSN1pGV0pGdjNoWjFuSDkrUT09",
    "lan_id": "98006755444",
    "policyIssuance": [
        {
            "plan_name": "Hospi Cash",
            "certificate_number": "",
            "ProposalNumber": "1506230001",
            "policy_start_date": "2023-06-15 00:00:00",
            "policy_expiry_date": "2023-06-15 00:00:00"
        },
        {
            "plan_name": "Group Personal Accident",
            "certificate_number": "",
            "ProposalNumber": "1506230002",
            "policy_start_date": "2023-06-15 00:00:00",
            "policy_expiry_date": "2023-06-15 00:00:00"
        }
    ],
    ...
    "session_arr": [
        "468.12",
        "RmlqUUlSN1pGV0pGdjNoWjFuSDkrUT09",
        "633",
        "d0ViSUVXYjlIYm5weG9COEczTFB0dz09",
        "amE5dHg3ZHZWcHMwSnl0R2NNQXlWQT09"
    ]
}';
        $_POST=json_decode($post_data,true);

        $response=json_decode($response,true);
        //print_r($response['Premium']['data']['Applicant']);die;
        $respp= $this->send_policy_request_to_ic($_POST,$response);
    }
