	...
	function fetch()
	{
		...
		if(!empty($dataListing['Data']['query_result']) && count($dataListing['Data']['query_result']) > 0)
		{
			for($i=0;$i<sizeof($dataListing['Data']['query_result']);$i++)
			{
				...
				$actionCol = "";
				if(in_array('PermissionAdd',$this->RolePermission)){
					//$actionCol .='<a href="permission/addEdit?text='.rtrim(strtr(base64_encode("id=".$dataListing['Data']['query_result'][$i]['perm_id'] ), '+/', '-_'), '=').'" title="Edit"><span class="spn-9"><i class="ti-pencil"></i></span></a>';
				}
				if(in_array('PermissionAdd',$this->RolePermission)){
